...
use crate::result::WiimoteDeviceError;
...
pub fn read_16_bytes_sync(
    ...
    for _i in 0..RETRY_COUNT {
        let input_report = match wiimote.read_timeout(READ_TIMEOUT) {
            Ok(report) => report,
            Err(WiimoteError::WiimoteDeviceError(WiimoteDeviceError::MissingData)) => continue,
